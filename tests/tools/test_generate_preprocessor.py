...
from tools.generate_preprocessor import load_html, load_few_shots, generate_preprocessor


# Tests for load_html

@patch("tools.generate_preprocessor.logger.debug")
@patch("tools.generate_preprocessor.fetch_page")
...
def test_load_few_shots_success(mock_debug):
    ...
    """
    with patch("tools.generate_preprocessor.open", mock_open(read_data=mock_json_content)):
...
@patch("tools.generate_preprocessor.logger.error")
@patch("tools.generate_preprocessor.open")
...
@patch("tools.generate_preprocessor.os.makedirs")
@patch("tools.generate_preprocessor.open", new_callable=mock_open)
@patch("tools.generate_preprocessor.logger.info")
@patch("tools.generate_preprocessor.logger.debug")
@patch("tools.generate_preprocessor.requests.post")
@patch("tools.generate_preprocessor.load_few_shots")
@patch("tools.generate_preprocessor.load_html")
def test_generate_preprocessor_success(mock_load_html, mock_load_few_shots, mock_post, mock_debug, mock_info, mock_file_open,
                                 mock_makedirs):
    """
    ...
    mock_response.json.return_value = {
        "choices": [
            {
                "message": {
                    "content": "```python\ndef preprocessor_factcheck(html_content):\n    pass\n```"
                }
            }
        ]
    }
    ...
    generate_preprocessor("https://test.com", "TestPortal")
    ...
    handle.write.assert_called_once_with("def preprocessor_factcheck(html_content):\n    pass")
    mock_debug.assert_called_once()
    assert "Sending generation request to KIConnect API" in mock_debug.call_args[0][0]
    mock_info.assert_called_once()
    assert "Successfully generated preprocessor for testportal" in mock_info.call_args[0][0]


@patch("tools.generate_preprocessor.logger.error")
@patch("tools.generate_preprocessor.load_html")
def test_generate_preprocessor_html_fail(mock_load_html, mock_error):
    """
    ...
    mock_error.assert_called_once()
    assert "Skipping preprocessor generation for testportal" in mock_error.call_args[0][0]
...
@patch("tools.generate_preprocessor.load_few_shots")
@patch("tools.generate_preprocessor.load_html")
def test_generate_preprocessor_api_fail(mock_load_html, mock_load_few_shots, mock_post, mock_error):
    """
    ...
    mock_error.assert_called_once()
    assert "Failed to generate preprocessor for testportal" in mock_error.call_args[0][0]